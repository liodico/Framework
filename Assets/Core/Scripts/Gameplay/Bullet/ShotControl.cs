...
public class ShotControl : MonoBehaviour
{
    ...
    {
        // "Set a shot pattern component (inherits BaseShot)."
        [FormerlySerializedAs("_ShotObj")]
        public BaseShot m_shotObj;
    ...
    private IEnumerator Start()
    {
        if (m_startOnAwake)
        {
            ...
            {
                yield return Util.WaitForSeconds(m_startOnAwakeDelay);
            }
            StartShotRoutine();
        }
    }
    ...
    private IEnumerator ShotCoroutine()
    {
        ...
        while (true)
        {
            ...
            {
                m_workShotInfoList[nowIndex].m_shotObj.SetShotControl(this);
                m_workShotInfoList[nowIndex].m_shotObj.Shot();
            }

            if (0f < m_workShotInfoList[nowIndex].m_afterDelay)
            {
                yield return Util.WaitForSeconds(m_workShotInfoList[nowIndex].m_afterDelay);
            }
